...
- Models are defined in `Sources/PrimitiveAppTemplate/Models/models.toml` — or, when this client shares one Primitive app with another (a web client in a sibling directory), in the file named by `Sources/PrimitiveAppTemplate/bao-codegen.json` (`{"input": "../../../models/models.toml"}`, resolved against the app's source directory). ONE schema per app either way: its TOML keys are the wire field names, so a second copy that drifts orphans the other one's records. `swift build` runs codegen automatically, and the Xcode targets run it from a pre-build phase, so every build path compiles the current schema. Adding a NEW model also needs the new file listed in the Xcode project before a build can see it — run `./run-ios.sh`, or the pair it runs (`bash scripts/codegen.sh && bash scripts/regenerate-project.sh`), since `xcodegen` can only list a file that already exists; the regeneration emits the models before it runs `xcodegen`, so on a fresh clone it covers them on its own. Until then an Xcode build fails naming the file. NEVER edit files under `Models/Generated/` — they are regenerated on every build. See `primitive guides get models --language swift`.
- **Generated code is code.** All four generated classes — model types under `Sources/PrimitiveAppTemplate/Models/Generated/`, workflow factories under `Sources/PrimitiveAppTemplate/Workflows/Generated/`, database types under `Sources/PrimitiveAppTemplate/Databases/Generated/`, server-function invokers under `Sources/PrimitiveAppTemplate/Functions/Generated/` — are **committed** and reviewed like any other source, and **every build path regenerates them**: `./build.sh`, `./run.sh`, `./run-ios.sh`, `scripts/smoke-test.sh`, `./archive.sh`, the fastlane lanes, and every Xcode entry point (Run, Profile, Archive, a bare `xcodebuild`). They all run `bash scripts/codegen.sh` — the one entry point for all four codegens — directly or through `scripts/regenerate-project.sh`. NEVER edit those files by hand. Change a schema, build, and the regenerated output is a working-tree diff you commit **with** the change. That is the whole enforcement: there is no drift check, no pre-commit hook and no CI step, and `./archive.sh` has no release-time codegen policy of its own.
- The workflow, database and server-function halves read the synced TOMLs (`<project root>/primitive/<env>/`) for the environment the CLI itself would resolve, so an app that has synced none of them generates none and needs no CLI at all.

## Shipping

- **TestFlight and the App Store go through the fastlane lanes, NEVER through `./archive.sh`.** `bundle exec fastlane ios beta` archives, exports and uploads a TestFlight build; `bundle exec fastlane ios release` uploads an App Store build — it does NOT start review, so tell the user to complete the version's metadata in App Store Connect and submit it for review by hand; `bundle exec fastlane bump type:patch` gets the new build number Apple requires for each upload. Run `bundle install` once first. The iOS lanes sign entirely from an App Store Connect API key in `fastlane/.env` — they authenticate the archive with it and fetch the distribution certificate and provisioning profile through it — so they need no Apple ID signed into Xcode and no certificate in the keychain (#3008).
- `./archive.sh {ios|mac|dmg}` is the **Xcode-account** path, not the TestFlight path: it signs through the Apple ID in Xcode → Settings → Accounts and never sees the API key, so on a machine set up as `fastlane/.env.example` prescribes it fails with `error: No Accounts: Add a new account in Accounts settings.` (#3416). That is the machine state, not the project — reach for the beta lane instead. Its `dmg` mode (Developer ID `.app` for direct macOS distribution) has no lane-based equivalent worth avoiding, and `fastlane mac beta` signs through Xcode's account too, so macOS TestFlight does need that Apple ID.
- Distribution setup — Team ID in `project.yml`, creating the API key, registering the app on App Store Connect — is in `docs/README.md` ("Distribution") and, in full, `primitive guides get deploying --language swift`. NEVER commit `fastlane/api_key.p8` or `fastlane/.env`.
...
**If this app has a web counterpart, that link should be an https URL, not the custom scheme (#2982).** Add `"webUrl": "https://<your-web-domain>"` to the environment in `primitive/config.json` (`primitive env add` takes `--web-url` when it CREATES an environment; it cannot edit one that already exists). It must be a bare origin — https, or http on `localhost`/`127.0.0.1` — and the resolve script carries it into `primitive.json`, `PrimitiveAppState.initialize()` sets it as `client.links.appBaseURL`, and the manager then sends `https://<your-web-domain>/oauth/callback` as the email's redirect target with no code change. Tapped on a device with this app installed it opens the app (a universal link); anywhere else it is an ordinary web sign-in. Allow-list that https URL exactly as below, serve `apple-app-site-association` from the web domain (the Vue template ships the file and the checklist under `public/.well-known/`), and enable the `applinks:` entitlement in `project.yml` last — the commented block there says why that order matters. `.onContinueUserActivity(NSUserActivityTypeBrowsingWeb)` in `ContentView` is what receives such a link on macOS and on hand-off.

To send a custom-scheme LINK instead (an app with no web client), two things have to be true, and each missing piece fails differently:

1. **Allow-list the URI.** Merge `<scheme>://auth/magic-link` into the existing `[auth].emailRedirectUris` array in the app's `primitive/<env>/app.toml` (run `primitive config pull --only app` first if that file isn't there yet), then `primitive config push --only app` — never a bare replacement array; `app.toml` is the whole app-settings truth on push. Missing → EVERY email sign-in request fails 400 `Invalid redirect URI`.
2. **Set `authManager.sendsEmailSignInLink = true`** before requesting the email. Missing → the email is code-only (no error). A manager constructed with an explicit `callbackScheme:`, or an app with a `webUrl` configured, already defaults to `true`; setting it to `false` forces code-only either way.
