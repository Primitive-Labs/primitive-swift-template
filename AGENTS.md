...
- The workflow and database halves read the synced TOMLs (`<project root>/primitive/<env>/`) for the environment the CLI itself would resolve, so an app that has synced neither generates neither and needs no CLI at all.
