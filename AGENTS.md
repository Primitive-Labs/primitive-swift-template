...
- **Generated code is code.** All three generated classes — model types under `Sources/PrimitiveAppTemplate/Models/Generated/`, workflow factories under `Sources/PrimitiveAppTemplate/Workflows/Generated/`, database types under `Sources/PrimitiveAppTemplate/Databases/Generated/` — are **committed** and reviewed like any other source, and **every build path regenerates them**: `./build.sh`, `./run.sh`, `./run-ios.sh`, `scripts/smoke-test.sh`, `./archive.sh`, the fastlane lanes, and every Xcode entry point (Run, Profile, Archive, a bare `xcodebuild`). They all run `bash scripts/codegen.sh` — the one entry point for all three codegens — directly or through `scripts/regenerate-project.sh`. NEVER edit those files by hand. Change a schema, build, and the regenerated output is a working-tree diff you commit **with** the change. That is the whole enforcement: there is no drift check, no pre-commit hook and no CI step, and `./archive.sh` has no release-time codegen policy of its own.
- The workflow and database halves read the synced TOMLs (`<project root>/.primitive/sync/<env>/<appId>/`) for the environment the CLI itself would resolve, so an app that has synced neither generates neither and needs no CLI at all.
