...
- This project uses **project mode**: a checked-in `.primitive/config.json` defines named Primitive environments (e.g. `dev`, `prod`), each binding an `apiUrl` and `appId`. It is the SINGLE place those are typed. Per-environment tokens live in `.primitive/credentials.json` and this machine's selection in `.primitive/local.json` (both gitignored). `primitive init` scaffolds a `dev` environment; if the config file is missing, create it with `primitive env add dev --api-url <url> --app-id <appId>`.
